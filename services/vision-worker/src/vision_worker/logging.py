...
Identical to `media_gateway.logging` and `application_memory.logging` --
fully generic (parameterized by `service`/`version`), copied rather than
shared across services to keep each service's dependency graph independent.
"""
...
#: `?token=...` in a URL. The overlay socket authenticates in the query string
#: because a browser cannot set headers on a WebSocket, and the value reaches
#: anything that logs a request line. Opaque tokens match no JWT shape.
