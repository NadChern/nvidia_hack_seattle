...
from agent.logging import REDACTED, JsonFormatter, RedactionFilter
...
def emit(**extra: object) -> dict[str, object]:
    ...
    formatted = JsonFormatter(service="agent", version="0.1.0").format(record)
...
def test_bytes_are_never_serialized() -> None:
    """Raw payloads collapse to a byte count, never content.

    The guarantee holds even if a call site passes bytes as an ``extra`` field
    by mistake.
    """
