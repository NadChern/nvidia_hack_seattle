...
`services/agent` understands the transcript, extracts intent and object name, calls the bounded deterministic Memory tool, and formulates the answer. Memory's complete query result remains authoritative: a six-rule deterministic guard rejects unsupported locations, missing uncertainty, lost ambiguity, empty replies, and model text produced without a tool call. A veto returns Memory's canonical `spoken_answer` rather than failing the request. The service supports console push-to-talk and an opt-in wake-prefix listener that returns synthesized PCM through the Media Gateway; barge-in remains explicitly out of scope.
