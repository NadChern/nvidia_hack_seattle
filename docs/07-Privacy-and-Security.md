...
- A successful claim returns an HMAC-signed, expiring device credential scoped to one `device_id`. For the wearer it authorizes session creation, refresh/deletion of that device's own session, its own HUD event socket, and a manual trigger. A paired helper credential may additionally list global pending assist requests, receive the assist event stream, accept a request, and mint the accepted session's helper grant. It still cannot list ordinary sessions, mint viewer grants, publish HUD events, read inference relays/status, or administer sessions.
...
- Remote-assist request and event discovery is currently global to every valid paired helper credential, not scoped to one wearer. The helper app publishes microphone audio only, but the tested LiveKit server rejected a server-enforced microphone-source grant, so the current room token temporarily permits any publish source. Camera remains disabled in the shipped helper client and helper tracks are excluded from inference ingest, but client discipline is not a security boundary. Per-wearer helper authorization and restoration of server-side microphone-only grants are required before production use.
