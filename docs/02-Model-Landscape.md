...
| Identity verification | [SAM 3.1](https://github.com/facebookresearch/sam3) | Exemplar-prompted confirmation of *which* instance ("my keys", not any keys), run selectively on a confirmed candidate's window | Downgraded from continuous critical-path use: SAM 3.1's published throughput (32fps object-multiplexed) is measured on an H100; this project's development and deployment hardware are unmeasured and likely much lower |
...
candidate event window + evidence ring
  ...
  ├── SAM 3.1 identity confirmation, selectively, on the window only
  ↓
...
### SAM 3.1: selective identity confirmation

Use text prompts such as `keys`, `wallet`, or `blue medication bottle`, plus exemplar images for personal objects, run on a confirmed candidate's evidence window rather than continuously on the stream (see "YOLOE: detector candidate" below for why). SAM can detect, segment, and track prompted concepts, but it does not determine persistent user-object identity, action semantics, room identity, or trusted memory state. Tracker IDs — YOLOE's own or SAM's — are scoped to a media epoch and must not survive reconnects.
