...
| Personal-object identity | [NVIDIA C-RADIOv4-SO400M](https://huggingface.co/nvidia/C-RADIOv4-SO400M) masked embeddings + Qwen3-VL escalation | Resolve *which physical instance* ("my keys", not any keys) once per track; store pooled vectors and reference crops | Adopted with constraints from the keys-only Phase-0 probe: F1 0.917 (22/25 positives, 24/25 negatives), +0.167 over CLIP; thresholds remain demo-specific until masked enrollment and held-out evaluation. Qwen handles only the near-threshold reject decision |
...
YOLOE's segmentation checkpoint supplies an in-process object mask. The same mask-to-crop transform is used for enrollment and matching, then C-RADIOv4 produces a distilled-teacher summary vector and mask-weighted spatial vector. Matching takes the best reference view for each query frame and averages across query frames. Clear matches must pass cosine and same-label runner-up margin gates; only the empirical overlap band reaches Qwen3-VL with stored reference crops for a direct same-instance reject decision. Identity is cached for one track and annotates events without vetoing them. See [Identity Probe Results](spikes/identity-probe/RESULTS.md). Tracker IDs remain scoped to a media epoch and never become stable identity.
